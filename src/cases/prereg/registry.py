"""Versioned preregistration registry (protocol §2.2).
...
class PreregRegistry:
    ...
    def assert_scoreable(self, cell_id: str) -> None:
        ...
        A cell that was never registered in the P0 registry is blocked as
        well — only preregistered, unblocked cells may run scored."""
        if not self._finalized:
            raise FrozenRegistryError("registry not finalized; publish P0 before scored runs")
        content = self._document["content"]
        if cell_id not in content["cells"]:
            raise BlockedCellError(f"cell '{cell_id}' is not registered in the P0 registry")
    ...
    @property
    def published_hash(self) -> str | None:
        """The registry_hash once finalized; None before publication."""
        if not self._finalized:
            return None
        return str(self._document.get("registry_hash"))
