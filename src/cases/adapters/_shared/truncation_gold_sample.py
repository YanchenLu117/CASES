...
Hardened (openai SDK without explicit timeout can hang on a stalled upstream
...
Usage:
    PYTHONPATH=src python -m cases.adapters._shared.truncation_gold_sample \
...
def run_sample(rows: list[dict], model: str, max_tokens: int = 32768,
    ...
    client = OpenAI(
        api_key=os.environ["CASES_LLM_KEY"],
        base_url=os.environ.get("CASES_GLM_BASE_URL", "http://localhost:8000/v1"),
