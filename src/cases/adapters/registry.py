...
Mirrors ``cases.llm.registry`` / ``cases.recovery.registry``: benchmark/loop code
...
SINGLE REGISTRATION AUTHORITY (P1 cleanup): this module's
...
available_adapters(): hypospace, bh, gb1
...
def _bh_factory(cfg: Mapping[str, Any] | None) -> "TaskAdapter":
    # NOTE (P0): BH solution arm is config-driven.  The frozen primary is
...
def _register_builtins() -> None:
    ...
    # requires a dedicated torch/rdkit env (see src/cases/adapters/pievo docs).
