"""Core — Algorithm 1: the coupled fast+slow persistent-loop driver.

This module is additive.  It turns the three-operation facade
(:class:`~cases.core.model.CASESModel`) plus the representation layer into a
...
The loop is the *orchestrator*: it never calls ``CASESLoop.propose`` (which is
...
from .model import EvidenceRecord, CASESModel
...
def default_implementer(model: CASESModel):
...
def default_evaluator(model: CASESModel):
...
class CASESAgentLoop:
    """Algorithm 1 — coupled fast+slow persistent-loop driver.

    Parameters
    ----------
    model:
        The :class:`CASESModel` facade whose primitive methods the loop drives.
    proposer:
...
__all__ = ["CASESAgentLoop", "default_implementer", "default_evaluator"]
