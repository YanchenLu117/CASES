"""Core — logic-governed neuro-symbolic representation induction.

This module is purely additive.  It gives the previously-unwritten core objects
of the Core revision (proposal §5, §7, §11, §12):
...
Permanent state  S = (D_t, P_t, C_t, Gamma_t, B_t) is completed here by the
representation specification ``P_t`` and its admission (logic + fidelity).

Design constraints
------------------
- Additive: does not rewrite the frozen core.  It *consumes* the
...
- Paradigm-flagged: the four representation-paradigm arms (V1 fixed embedding,
  fixed-symbolic, -minus-Logic, Full) are first-class so the
...
def _run_coro(coro):
    ...
    if _worker_loop is None or not _worker_loop.is_running():
        ...
        _worker_thread = threading.Thread(target=_worker_loop.run_forever, name="cases-inducer", daemon=True)
...
class RepresentationParadigm(str, Enum):
    ...
    FIXED_EMBEDDING  : fixed embedding into a fixed latent space; no induced
                          task structure; no logic gate.
    FIXED_SYMBOLIC   : the frozen expert compiler (K o V o P) into a
                          predefined schema; strict symbolic verification only.
    INDUCED_NO_LOGIC         : LLM-induced representation with NO logic gate and NO
                          fidelity gate (isolates the value of the logic gate).
    FULL             : LLM-induced + logic gate + fidelity gate (complete).
    """

    FIXED_EMBEDDING = "fixed_embedding"
    FIXED_SYMBOLIC = "fixed_symbolic"
    INDUCED_NO_LOGIC = "induced_no_logic"
    FULL = "full"

    @property
    def runs_logic_gate(self) -> bool:
        return self is RepresentationParadigm.FULL
    ...
    def is_adaptive(self) -> bool:
        # arms induce/revise; the non-adaptive paradigms do not.
        return self in (RepresentationParadigm.INDUCED_NO_LOGIC, RepresentationParadigm.FULL)
...
class RepresentationRuntime:
    ...
    C_t to drive B_t in :meth:`~cases.core.model.CASESModel.recover`.
    """
...
class RepresentationInducer:
    ...
    This is the **neural (proposal) authority** side of the
