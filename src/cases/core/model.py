"""CASESModel — the three-operation facade (FROZEN contract, §3.1).

    cases.update(evidence_batch)
    space_state = cases.recover()
    readout     = cases.readout(space_state, token_budget=...)
...
class LanguageCorrespondence:
    ...
    Core proposal §10: the correspondence must be two-way.
...
async def _ground_one(
    ...
    the same semantics as ``CASESLoop.ground``."""
...
def _worker() -> asyncio.AbstractEventLoop:
    ...
    if _worker_loop is None or not _worker_loop.is_running():
        ...
        _worker_thread = threading.Thread(
            target=_worker_loop.run_forever, name="cases-facade", daemon=True
        )
...
class _MeteredLLM:
    ...
    Wraps the :class:`LLMProvider` and forwards its public surface
    ...
    An :class:`LLMBackend` (whose ``generate`` returns a ``GenerationResult``)
    is wrapped too if passed directly; the facade records its token metering.
    """

    __slots__ = ("_inner", "_model")

    def __init__(self, inner, model: "CASESModel") -> None:
        self._inner = inner
        self._model = model
    ...
    def _meter(self, result: Any) -> None:
        ...
        # GenerationResult: has input_tokens / output_tokens attributes.
        if getattr(result, "input_tokens", None) is not None:
            self._model.record_generation(result)
            return
        # LLMResponse: usage is on ``.usage``.
...
class CASESModel:
    ...
    paradigm:
        The representation-paradigm arm (Core §A.5 / Claim C4).  Defaults to
        ``FULL`` (logic gate + fidelity gate active).  INDUCED_NO_LOGIC disables
        the dual gate; the fixed non-adaptive paths.
    epsilon_f:
    ...
    def __init__(
        ...
        *,
        ...
        paradigm: RepresentationParadigm = RepresentationParadigm.FULL,
        epsilon_f: float = 0.5,
        ...
        self.eta = eta
        # representation layer (proposal §5, §7, §11-§12, §21-§25).
        ...
        # stays callable as the contract operation.
    ...
    def recover(self, features: Mapping[str, Any] | None = None) -> SolutionSpaceState:
        ...
        ``space_state = cases.recover()`` genuinely fits; a features-requiring
    ...
    def _fit_posterior(self, recovery_input) -> tuple[str, Any]:
        ...
        ``mean`` for an empty graph — so the happy-path ``recover()`` never
    ...
    # -- representation layer (P_t, L_tau, F_t, dual gate, revision) ------
    ...
    def admit_representation(
        ...
        ``F(P_t, H) >= epsilon_f``.  Full enforces both; INDUCED_NO_LOGIC enforces
        neither contract (the "value of logic" ablation arm); the non-adaptive paradigms are
    ...
    def representational_state(self) -> dict[str, Any]:
        """The completed persistent state  S = (D_t, P_t, C_t, Gamma_t, B_t)
    ...
    def check_revision(self) -> RevisionSignal:
        """Scan the four triggers .  Never fires for the non-adaptive paradigms."""
    ...
    def record_generation(self, generation_result) -> None:
        """Fold a :class:`~cases.llm.base.GenerationResult` into the ledger.

        Convenience for the ``LLMBackend`` path: reads ``input_tokens`` /
        ...
        facade (and by the metering wrapper for ``LLMProvider`` paths).
        """
