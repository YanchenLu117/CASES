"""SpaceReadout — the v1.1 formal component (Core).
...
readout* from that state.  v1.1 reopens the core **only** for Space Readout:
...
The readout still never selects candidates — CASES remains a representation
layer, not a controller.

Frozen v1.1 boundaries (per governance):
...
class SolutionFamily:
    """One section-5.3 view of the recovered solution space ( additive).
...
class ReadoutPacket:
    ...
    metadata: Mapping[str, Any] = field(default_factory=dict)
    # additive: section-5.3 solution-family views (empty when no posterior
...
class SpaceReadout(ABC):
    ...
    def build_with_posterior(
        ...
        """Additive path: ``build`` plus, when a fitted ``posterior`` is
...
class CoverageFrontierReadout(SpaceReadout):
    ...
    def build_with_posterior(
        ...
        """Additive path: ``build`` plus section-5.3 solution-family views.
