"""this baselines — unified comparator registry (EXPERIMENT_PLAN §5).
...
given history — measured on the SAME recovery/utility metrics as CASES):
  ...
  - a4_full_cases      : A4 — the full CASES loop (marker: run-driver runs the loop;
                         factory returns an CASESModel builder)

SPECIALIST block:
  - gp_bo, dkl_bo, alde, random, greedy_hillclimb

Honesty: the main-matrix arms A0-A3 and the specialist gp_bo/a3 are REAL
deterministic proposers; A4 is a marker for the full CASES loop (run-driver).
...
adapter boundary (cases/adapters/aiscientist, baselines/native_arms), not here.
"""
...
class SummaryProposer(_Proposer):
    ...
    (same budget as CASES's serialized state — the memory-compression baseline).
...
def _register_builtins() -> None:
    ...
    register_baseline("a4_full_cases", lambda cfg=None: _UnavailableProposer("a4_full_cases", "marker: run-driver runs the full CASES loop"))
