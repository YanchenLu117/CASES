"""this Native AI-scientist / host arms (EXPERIMENT_PLAN §5.2 / §6.2).
...
class Host(str, Enum):
    SCIEXPLORER = "sciexplorer"   # four arms (Native/Summary/CASES-State/Full)
    MADE = "made"                 # four arms
    PIEVO = "pievo"               # three arms (Native/Summary/CASES-plug-in)
...
_NATIVE_ARMS: Mapping[Host, NativeArmSpec] = {
    Host.SCIEXPLORER: NativeArmSpec(
        Host.SCIEXPLORER, ("native", "summary", "cases_state", "full"),
        ...
        {"native": "official loop unchanged", "summary": "token-matched prose summary context only",
         "cases_state": "read-only CASES state in fields/round message", "full": "CASES channel + target region injected per round"}),
    Host.MADE: NativeArmSpec(
        Host.MADE, ("native", "summary", "cases_state", "full"),
        ...
        {"native": "official pipeline unchanged", "summary": "token-matched prose summary context only",
         "cases_state": "state['cases_readout'] added for official planner/scorer", "full": "CASESPlanner + CASESFieldScorer replace official proposer/selector"}),
    Host.PIEVO: NativeArmSpec(
        Host.PIEVO, ("native", "summary", "cases_plugin"),
        ...
        {"native": "official multi-agent loop unchanged", "summary": "token-matched evidence summary only",
         "cases_plugin": "CASES state as control surface for the discovery loop (C5)"}),
}
...
AI_SCIENTIST_SYSTEMS: Mapping[str, AIScientistSpec] = {
    "S1": AIScientistSpec("S1", "SciExplorer", "github.com/MaxNaeg/SciExplorer", "SciExplorer-Physics",
                          "four-arm Native/Summary/CASES-State/Full via run_exploration public interface; smoke_gate_ok"),
