...
``--decision-registry FILE``
    ...
    must pass the repo's canonical verifier (``cases.protocol.registry``)
    when importable.
...
Usage (A-line style, before a confirmatory launch)::
    ...
    PYTHONPATH=src python -m cases.infra.preflight \\
        ...
        --disk /data=90 --report work/A_hypospace/preflight_report.json
"""
...
def verify_decision_registry(decision_path: Path) -> List[Check]:
    ...
    Repo convention (src/cases/protocol/registry.py): a registry document is
    *self-hashing* — it carries ``registry_hash`` = sha256 over its
    ...
    try:
        from cases.protocol.registry import verify_registry_hash as _verify
    except Exception:
...
def count_processes(pattern: str) -> int:
    ...
    Never counts the preflight tooling itself (any ``cases.infra``
    ...
    for line in result.stdout.splitlines()[1:]:
        ...
        if len(parts) < 2 or not parts[0].isdigit():
            continue
        pid, args = parts
        if pid == my_pid or "cases.infra" in args:
            continue
        if pattern in args:
            count += 1
    return count
...
def run_preflight_checks(
    *,
    ...
    endpoint_model: str = "zai-org/GLM-5.3-Flash",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--endpoint-model", default="zai-org/GLM-5.3-Flash")
