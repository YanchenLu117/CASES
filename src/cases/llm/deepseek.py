...
class DeepSeekProvider(LLMProvider):
    def __init__(
        ...
        *,
        ...
        self._model = model
        self._base_url = base_url or os.environ.get("CASES_LLM_BASE_URL", "http://localhost:8000/v1")
...
# ---------------------------------------------------------------------------
#  — DeepseekV4Flash (LLMBackend) over the OpenAI-compatible vLLM gateway
# ---------------------------------------------------------------------------

# (appended below the DeepSeekProvider module; original code above unchanged)
...
def _default_api_key() -> str:
    ...
    Never a literal in source.  Order: CASES_LLM_API_KEY, then DEEPSEEK_API_KEY.
    """
    for name in ("CASES_LLM_API_KEY", "DEEPSEEK_API_KEY"):
        value = os.environ.get(name)
        if value:
            return value
    raise ValueError(
        "DeepseekV4Flash requires an API key: set CASES_LLM_API_KEY or "
        "DEEPSEEK_API_KEY (see configs/llm_endpoints.example.json)."
    )


# Default gateway for the deployed vLLM endpoint (FROZEN, per architecture).
DEFAULT_V4FLASH_BASE_URL = "http://localhost:8000/v1"
...
class DeepseekV4Flash(LLMBackend):
    ...
    vLLM gateway, using the openai SDK (present in the cases-core env).

    Configuration (see also configs/llm/deepseek_v4_flash.yaml):

        base_url          : default from CASES_LLM_BASE_URL
    ...
    def __init__(
        ...
        *,
        ...
        self._base_url = (
            base_url
            or os.environ.get("CASES_LLM_BASE_URL")
        ...
        try:
            ...
            raise ImportError(
                "DeepseekV4Flash needs the openai package (see cases-core env)"
            ) from exc
        # P1 fix: use the RESOLVED base_url (``self._base_url``), which folds in
        # the CASES_LLM_BASE_URL env override and the frozen default.  The raw
