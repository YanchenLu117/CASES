"""LLM registry — maps a MODEL name to an :class:`LLMBackend` factory ,
and keeps the provider registry for backward compatibility.

Existing benchmark/loop code continues to use :class:`LLMProvider` through the
``provider`` key.  The ``backend`` registry is the extensibility seam: adding
...
# ---------------------------------------------------------------------------
#  — backend registry (by model name)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
#  — provider registry (frozen, unchanged semantics)
# ---------------------------------------------------------------------------
...
def _register_builtins() -> None:
    ...
    # default model -> DeepseekV4Flash backend.
