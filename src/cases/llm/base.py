...
The ``LLMProvider`` abstraction used by the frozen TaskAdapter
``compile`` path — vendor-neutral ``LLMRequest`` / ``LLMResponse`` dataclasses.

The extensible ``LLMBackend`` contract and ``GenerationResult``
...
subclass registered by model name in ``cases.llm.registry``; core never imports
...
# ---------------------------------------------------------------------------
#  — LLMBackend (extensible contract) + GenerationResult
# ---------------------------------------------------------------------------
...
class LLMBackend(ABC):
    """Extensible vendor-neutral LLM contract .
