"""Unified statistical decision layer (Plan §3.1/§12, Detail §14).
...
def _paired_deltas(cases: Sequence[float], baseline: Sequence[float]) -> np.ndarray:
    a = _as_array(cases, "cases")
...
def paired_summary(
    ...
    *,
    ...
    deltas = _paired_deltas(cases, baseline)
...
def decide(
    ...
    *,
    ...
    summary = paired_summary(cases, baseline, n_boot=n_boot, n_perm=n_perm, seed=seed)
    deltas = _paired_deltas(cases, baseline)
