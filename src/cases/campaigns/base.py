"""Campaign engine — reusable finite-oracle experiment runner (CASES).
...
first-class, reproducible campaign engine:
  ...
  * an ``AcquisitionPolicy`` seam so baseline arms (random / greedy / history /
    scalar / DKL-BO) and the CASES arm all consume the *same* oracle budget
...
class AcquisitionPolicy(Protocol):
    ...
    stateful arms (CASES / history / scalar).
