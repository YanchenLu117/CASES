#!/usr/bin/env python
"""BH/GB1 scored-campaign runner (projects B=BH, C=GB1).

LLM track: GLM-5.3-Flash only, v3_1 endpoint (configs/llm_endpoints.json,
the configured OpenAI-compatible endpoint (see configs/llm_endpoints.example.json), max_tokens 8192
...
Protocol authority:
See docs/PROTOCOL.md (§2 registry, §6 common universe + AP endpoints,
§8 BH, §9 GB1, §13 caps, §14 statistics).  Phases (protocol ①):
    ...
    p0         freeze preregistration_registry.json (|S*|, legal-mask hash,
               ...
               assert_scoreable
    scored     arms x seeds under ResourceMeter; per-run JSON (schema
               compatible + protocol block), per-checkpoint score npz (§6 capture),
    ...
    aggregate  common-universe AP Recovery-AURC (§6), paired §14 stats, Holm
               within project x track, TOST gate vs frozen anchor results, gains
...
Arms = reruns of the anchors table (same harness/seeds/caps; anchor numbers are
regression anchors + TOST equivalence gate only):
  BH : cases_graph (model) | bo_gp, bo_dkl, ballet_level | random, hillclimb
  GB1: cases_hamming (model) | gb1_gpbo, gb1_ballet | random, hillclimb
  The GB1 ALDE specialist runs in a separate environment and is joined later.

Usage (from the project root):
  python scripts/run_bh_gb1.py --project bh  --phase pilot
  python scripts/run_bh_gb1.py --project bh  --phase p0
...
from cases.campaigns.base import (  # noqa: E402
    CampaignConfig,
    ResourceLedger,
    RunConfig,
    git_commit,
...
from cases.campaigns.recovery_eval import RecoveryEvalResult, aurc, evaluate_recovery  # noqa: E402
from cases.prereg.registry import PreregRegistry, verify_registry_hash  # noqa: E402
from cases.stats.decision import (  # noqa: E402
    DECISION_IMPROVES,
    decide,
    holm_adjust,
    paired_summary,
...
from cases.protocol.fairness import CAP_KEYS, ResourceMeter  # noqa: E402
...
# LLM endpoints: resolved from the environment, never hardcoded.  Provide an
# OpenAI-compatible endpoint via CASES_LLM_BASE_URL / CASES_LLM_API_KEY (see
# configs/llm_endpoints.example.json).  "glm" is the legacy banked-data track;
# any roster layer resolves via cases.llm.layers.
ENDPOINTS = {
    "glm": (os.environ.get("CASES_LLM_BASE_URL", "http://localhost:8000/v1"),
            os.environ.get("CASES_LLM_MODEL", "zai-org/GLM-5.3-Flash"),
            os.environ.get("CASES_LLM_API_KEY", "")),
}
MAX_TOKENS = {"glm": 8192}
...
def _layer_choices() -> list[str]:
    try:
        from cases.llm.layers import roster as _roster
        return list(_roster())
    except Exception:
        return []


def _resolve_model(key: str) -> tuple[str, str, str, int]:
    """(base_url, model_id, api_key, max_tokens) for a legacy key or a layer."""
    if key in ENDPOINTS:
        return (*ENDPOINTS[key], MAX_TOKENS.get(key, 8192))
    from cases.llm.layers import LayerNotConfigured, get_layer, get_layer_secret
    try:
        cfg = get_layer(key, strict=True)
    except LayerNotConfigured as exc:
        raise SystemExit(f"layer {key!r} not wired: {exc} (see cases.llm.layers docs)")
    return (cfg.base_url, cfg.model, get_layer_secret(cfg), cfg.max_tokens)
...
MIN_SEEDS = {"bh": 10, "gb1": 30}   # Detail §8.2 BH=10 paired seeds, §9.2 GB1=30
                                    # (frozen 2026-08-31: BH 10 paired seeds × 240
                                    # steps; pilot power may propose more — any
                                    # extension is a prereg amendment BEFORE p0
...
GB1_ONEHOT_POOL_CAP = 25_000  # harness value (documented O(n^2) feasibility cap)
...
# Per-worker cache: official CLADE 'AA' encoding for the candidate pool.
...
# Stage-1.5 specialists (protocol §8.4/§9.4):
...
DEFAULT_ARMS = {
    "bh": ["cases_graph", "gryffin_plain", "gryffin_desc", "bo_gp", "ballet_level",
           "bo_dkl", "random", "hillclimb"],
    "gb1": ["cases_hamming", "gb1_clade", "gb1_gpbo", "gb1_ballet", "gb1_alde",
            "random", "hillclimb"],
}
PRIMARY_ARM = {"bh": "cases_graph", "gb1": "cases_hamming"}
# Confirmatory Holm family = protocol specialists only (Detail §8.4/§9.4).
# random/hillclimb are proposal-policy context arms: their rows are an
# old-data join with metric n/a (BH_GB1_MAIN_AGG), so the frozen decision
# (2026-08-31, pre-outcome) keeps them out of the confirmatory family —
# their contrasts are computed but
# labeled descriptive (Holm p = raw p, family tag "descriptive").
# Family amendment (pre-outcome, before any scored run / p0 freeze):
...
# trimming is a steering-committee ruling at the p0 freeze
# (DECISION_STAGE15_ARMS_20260831).
...
ANCHORS = {  # frozen numbers (V8_REGRESSION_ANCHORS.md) — anchors + TOST gate only
    "bh": {"cases_graph": 0.8112, "bo_gp": 0.5274, "ballet_level": 0.4843, "bo_dkl": 0.2683},
    "gb1": {"cases_hamming": 7.8135, "gb1_gpbo": 7.5547, "gb1_ballet": 6.3605, "random": 4.2823},
}
BOOT_SEED = 20260829
...
def archived(name: str):
    ...
    moved; src/cases is frozen for benchmark engineers, so a patched copy is
    ...
    dst = cache / f"{name}.py"
    src = ROOT / "src" / "cases" / "campaigns" / "_archive" / f"{name}.py"
    if not dst.exists() or dst.stat().st_mtime < src.stat().st_mtime:
        text = _ARCH_FIX.sub(r"\1from cases.campaigns.\2 import", src.read_text())
        text = re.sub(r"(?m)^(\s*)from \.\.adapters\.", r"\1from cases.adapters.", text)
        dst.write_text(text)
...
# Dispatch ruling 17 (v9-matrix workspace isolation): per-model output root
# override. None = legacy layout work/<B_bh|C_gb1>/v8_runs (unchanged).
OUT_ROOT = None  # Optional[Path]


def runs_dir(project: str, phase: str) -> Path:
    if OUT_ROOT is not None:
        return OUT_ROOT / phase
...
def _gb1_worker_init(arms: tuple) -> None:
    ...
    hamm = GB1HammingProp(oracle) if "cases_hamming" in arms else None
...
# ---------------------------------------------------------------------------
# BH arms ( harness semantics + metering / §6 score capture)
# ---------------------------------------------------------------------------
...
# reactions exactly (verified 2026-08-31: 4,132 all-4-component rows − 177
...
class _BHMeasuredUniverse:
    ...
    Sensitivity universes, never primary (counts verified 2026-08-31):
      - all-4-component real wells ......... 4,132 rows, |S*|=207, gamma=83.048
        (the pre-verification choice; recorded as labeled sensitivity)
      - first-3-nonempty ( anchor uni.) .. 4,312 rows, |S*|=216, gamma=83.370
        (used only by v8_probe_wt.py Probe A for -anchor attribution and by
    ...
    pins the protocol-exact 3,955.  Veto point: the p0 registry hash freeze.
    """
...
def bh_run_one(task: tuple):
    """One BH (arm, seed) campaign: the harness semantics + metering/capture."""
    ...
    def v7_eval_at(budget: int):
        ...
        prob_map = None
        if arm.startswith("cases") and gp is not None and gp.trained:
...
def _gb1_solution_regions_knn(oracle, k: int = 5) -> dict[int, list[int]]:
    """Frozen outcome-blind S* family geometry ( harness: kNN on 80-d one-hot)."""
...
def _clade_official_sequence(oracle, pool: list, seed: int, save_dir: Path):
    ...
    once, with an honest label channel: the Fitness wrapper evaluates the
    ...
    96 + 4x96 = 480 budget equals the GB1 schedule exactly (§9.2/§9.4).
...
class _CladeReadout:
    ...
    def predict_probs_by_index(self, indices) -> np.ndarray:
        from cases.recovery.graph_matern import _norm_cdf
        idx = list(indices)
...
def gb1_run_one(task: tuple):
    ...
    from cases.campaigns.gb1_predict import GB1OneHotGP
    ...
    clade_cursor = 0
    clade_calls = 0
    if arm == "cases_hamming":
        pred = _W["hamm"]
        query_pool = pool
    elif arm in ("gb1_gpbo", "cases_onehot"):
    ...
    if project == "bh":
        ...
        return {
            ...
            "universe_note": "primary = Detail §8.1 protocol-exact 3,955 measured reactions "
                             ...
                             "release 2026-08-31.  Labeled sensitivities, never primary: "
                             f"all-4 {facts_all4['pool']} (|S*|={facts_all4['solution_set_size']}), "
                             f"-anchor first-3 {facts_first3['pool']} "
            ...
            "sensitivity_universes": {
                ...
                "v7_first3_anchor_only": {"rows": len(first3), **{k: v for k, v in facts_first3.items()
                                                                   if k in ("pool", "solution_set_size", "gamma")},
                                          "note": "anchor attribution / probe A only"},
            },
        }
    from cases.adapters.gb1.data import GB1Data, WILD_TYPE
    from cases.campaigns.gb1_oracle import GB1FiniteOracle
    data = GB1Data()
...
def build_registry(project: str, seeds: list[int], commit: str, model: str = "glm") -> PreregRegistry:
    ...
    reg = PreregRegistry(f"scored cell {cell} (engineer BC, {model} track)")
    reg.set("repository.commits", {"cases": commit})
    ...
    _ep = _resolve_model(model)
    reg.set("model.tool_config", {"llm_endpoint": _ep[0] + (" (llm_endpoints)" if model in ENDPOINTS else " (layer registry)"),
                                  "declared_model": _ep[1],
                                  "max_tokens": _ep[3],
    ...
    reg.set("adapters.identity_reports", {project: "cases.campaigns harness, rerun"})
    ...
    for arm in DEFAULT_ARMS[project]:
        ...
        reg.set_baseline(bid, {
            "official_identifier": f"cases.campaigns this arm {arm}",
            "commit_hash": commit,
            "container_hash": "venv-py3.10",
            "entry_point": "scripts/run_bh_gb1.py",
...
def make_tasks(project: str, phase: str, seeds: list[int], arms: list[str], args) -> list[tuple]:
    ...
    if phase == "scored":
        ...
        if getattr(args, "probe", False):
            # dispatch ruling 11: --probe = early-read prefix of the frozen
            # grid — first 3 paired seeds of the registry seed set x --arms.
            # No registry mutation, no seed-set override beyond the prefix;
            # same scored layout, so a later full scored run resumes over it.
            seeds = seeds[:3]
            print(f"[{project}:scored] PROBE seeds={seeds} (first 3 of registry "
                  f"seed_list; source: {runs_dir(project, 'p0') / 'preregistration_registry.json'})",
                  flush=True)
        ...
        for base in DESCRIPTIVE_BASELINES:
            ...
            for endpoint in PRIMARY_ENDPOINTS:
                ...
                contrasts[f"{primary}_vs_{base}:{endpoint}"] = {
                    ...
                    "decision": dec.label,
                    "family": "descriptive (-join context arm; metric n/a in this agg; "
                              "outside the Holm family)",
                    "n_failures": int(sum(seed_vals[base][s]["failed"] for s in seeds if s in seed_vals[base])),
                }

        # -- TOST equivalence gate vs frozen anchor results (paired by seed)
        tost = {}
        ...
        for arm in arms:
            ...
            if len(v7_vals) >= 6:
                ...
                tost[arm] = {"n_pairs": len(deltas),
                             ...
                             "gate": "anchor citable" if equiv else "USE_RERUN_ONLY"}
        summary = {
            "registry_hash": doc["registry_hash"],
...
def _write_gain_ledger(project, summary, seed_vals, seeds, metric_key, primary, arms, norm, n_sol,
                       model: str = "glm") -> None:
    lines = [f"# Gains Ledger — {project.upper()} (engineer BC, {model} track)", "",
    ...
    lines += ["", "## TOST equivalence gate vs frozen anchor results", "",
              "| arm | n_pairs | mean rerun−anchor | TOST within δ_eq | gate |", "|---|---|---|---|---|"]
...
def _fig_main_paired(project, arms, primary, vals, agg, anchor, metric_key) -> None:
    ...
    for a, v in anchor.items():
        ...
        ax.text(len(arms) + 0.42, v, f"this {a}={v}", fontsize=7, va="center", color="#444")
    ...
    ax.set_title(f"{project.upper()} rerun {metric_key} — n={len(common_seeds)} paired seeds, "
...
def _fig_utility(project, arms, seeds) -> None:
    ...
    for ax in axes:
        ax.set_xlabel("oracle labels")
        ax.legend(fontsize=7)
    fig.suptitle(f"{project.upper()} utility curves (rerun, n={len(seeds)})")
    fig.tight_layout()
    for ext in ("png", "pdf"):
...
def main() -> int:
    ...
    os.environ.setdefault("OPENBLAS_NUM_THREADS", "4")
    os.environ.setdefault("MKL_NUM_THREADS", "4")
    ap = argparse.ArgumentParser(description="BH/GB1 scored-campaign runner (projects B+C)")
    ...
    ap.add_argument("--arms", default="", help="comma list; default = full -main arm set")
    ap.add_argument("--seeds", type=int, default=0, help="ignored for scored (registry is authority)")
    ap.add_argument("--probe", action="store_true",
                    help="scored prefix: run only the FIRST 3 frozen paired seeds of "
                         "the active registry seed set x the --arms subset; same scored "
                         "output layout (resume-safe prefix of the frozen grid; registry "
                         "stays the authority, no seed-set override)")
    ap.add_argument("--model", default="glm",
                    choices=list(ENDPOINTS) + [k for k in _layer_choices() if k not in ENDPOINTS],
                    help="declared LLM track: legacy 'glm' (banked protocol) or any wired "
                         "layer (experiments/v9_layers/<name>/layer.env); numerical arms "
                         "make no LLM calls")
    ap.add_argument("--jobs", type=int, default=16)
    ap.add_argument("--commit", default="")
    ap.add_argument("--out", default="",
                    help="per-model output root override (ruling 17): e.g. "
                         "work/v9_matrix/<model>/<benchmark>; all pilot/p0/scored "
                         "artifacts of this invocation go under <out>/<phase>/...; "
                         "default = legacy layout work/<B_bh|C_gb1>/v8_runs "
                         "(unchanged when not passed)")
    args = ap.parse_args()
    if args.out:
        global OUT_ROOT
        o = Path(args.out)
        OUT_ROOT = o if o.is_absolute() else (ROOT / o)
    if args.probe and args.phase != "scored":
        ap.error("--probe applies to --phase scored only (pilot/p0 stay untouched)")
